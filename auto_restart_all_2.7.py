...
def logs(log_text):#日志输出
    logger = logging.getLogger(u'yzx')
    logger.setLevel(logging.INFO)
    formatter = logging.Formatter(u'%(asctime)s %(levelname)s %(message)s', datefmt=u'%Y-%m-%d %R:%S')
    if not logger.handlers:  # handlers属性，控制重复输出.因为logger的name被固定，所以当你第一次为logger对象添加FileHandler对象之后，如果没有移除上一次的FileHandler对象，第二次logger对象就会再次获得相同的FileHandler对象，即拥有两个FileHandler对象，最终造成打印两次，同样，如果此时没有立即移除上一次的FileHandler对象，第三次logger对象就会再次获得相同的FileHandler对象，即拥有三个FileHandler象，最终打印3次........
        file_out = logging.FileHandler(u'/home/www/logsfile/auto_restrt.log')  # 用于输出至文件
        # logger.setLevel(logging.INFO)
...
def mysql_conn(dbip,username,pws,dbname):#定义mysql_conn函数，ip,username,pws为形参
    try:
        ...
        logs(u"IP:%s Could not connect to MySQL server."%dbip)
    return connects
...
def http_status(appurl,app_name):  # 定义获取http请求的返回状态码函数
    ...
    try:
        ...
        if code == 200:
            ...
            logs(u'[%s]当前http请求状态码为:%s' % (app_name, code))
            return code
...
def app_info():#获取app的hostname,ip,port,application,cmds信息
    ...
    with connects.cursor() as cursor:
        ...
        cursor.execute(u'select hostname,ip,port,application,cmds from app_info where ip="%s" and open_status=1;' % host_ip)  # 查询数据库项目列表
        connects.commit()
        info = cursor.fetchall()
        if len(info) < 1:
            print u'当前服务器未在数据库配置应用参数信息！'
            logs(u'当前服务器未在数据库配置应用参数信息！')
        connects.close()
        return info
...
def cluster_status(app_name):#查询app_name是否集群
    ...
    with connects.cursor() as cursor:
        cursor = connects.cursor()
        cursor.execute(u'select count(1) from app_info where application="%s" and open_status=1;' % app_name)  # 查询数据库项目列表
        connects.commit()
...
def online_status(app_name):#查询app_name在线数率
    ...
    with connects.cursor() as cursor:
        cursor = connects.cursor()
        cursor.execute(u'select count(1) from app_info where application="%s" and status=1 and open_status=1;' % app_name)  # 查询数据库项目列表
        connects.commit()
        online_num = cursor.fetchone()

        cursor.execute(u'select count(1) from app_info where application="%s" and status=0 and open_status=1;' % app_name)  # 查询数据库项目列表
        connects.commit()
...
def update_status(status,host_ip,app_name): #更新状态
    connects = mysql_conn(dbip, username, pws, dbname)
    with connects.cursor() as cursor:
        cursor.execute(u"update app_info set status=%s where ip='%s' and application='%s' and open_status=1;" % (status,host_ip,app_name))  # 更新状态
        connects.commit()
        connects.close()
...
def update_scan_err_time(dates,host_ip,app_name): #更新last_scan_err_time
    connects = mysql_conn(dbip, username, pws, dbname)
    with connects.cursor() as cursor:
        cursor.execute(u"update app_info set before_scan_err_time=last_scan_err_time where ip='%s' and application='%s' and open_status=1;" % (host_ip, app_name))  # 更新before_scan_time
        connects.commit()
        cursor.execute(u"update app_info set last_scan_err_time='%s' where ip='%s' and application='%s' and open_status=1;" % (dates,host_ip,app_name))  # 更新更新last_scan_time状态
        connects.commit()
        connects.close()
...
def update_last_startup_time(start_time,times,http_code,host_ip,app_name): #更新last_startup_time
    connects = mysql_conn(dbip, username, pws, dbname)
    with connects.cursor() as cursor:
        cursor.execute(u"update app_info set before_startup_time = last_startup_time where ip='%s' and application='%s' and open_status=1;" % (host_ip, app_name))  # before_startup_time
        connects.commit()
        cursor.execute(u"update app_info set last_startup_time = '%s',startup_times = %s,http_code = %s where ip='%s' and application='%s' and open_status=1;" % (start_time,times,http_code,host_ip,app_name))  # 更新更新start_time,times,http_code
        connects.commit()
        connects.close()
...
def restart(restart_cmd,app_name,home_path,appurl,host_ip,msg_ip_port): #重启函数
    ...
    date_tmp = start_date.split(u':')
    start_date_5 = date_tmp[0] + u':' + unicode(int(date_tmp[1])+5)#启动时间加5分钟，方便sed根据时间过滤出startup in日志
    ...
    restart_repl = unicode(restart.stdout.read(), encoding=u'utf8')
    logs(restart_repl)
    while True:  # 获取到startup后退出循环
        logs(u'auto_restart-循环检验[%s]重启结果:%s次' % (app_name, counts))
        ...
        startup_repl = unicode(startup_logs.stdout.read(),encoding=u'utf-8')  # 转str
        ...
        http_code_rpel = unicode(http_code.stdout.read(),encoding=u'utf8')  # 状态码
        if len(startup_repl) > 10 and http_code_rpel == u'200':  # 获取到startup日志（正常startup in 日志超过10个字符），并且状态码正常执行更新sql
            repl_list = startup_repl.split(u' ')  # 分割日志
            start_time = repl_list[0] + u' ' + repl_list[1]  # startup in时间
            times = repl_list[-2]  # 耗时
            update_last_startup_time(start_time, times, http_code_rpel, host_ip, app_name)#更新重启时间
            update_status(u'1', host_ip, app_name)#更新状态，在线
            logs(u'%s Server Restart Successfully' % app_name)  # 日志记录重启
            break
        elif counts>5: #循环5次退出，发送失败信息
            # #短信告警接口
            logs(u'auto_restart-[%s]重启超时检测次数:%s' % (app_name, counts))
            error_date = time.strftime(u'%Y-%m-%d %H:%M:%S', time.localtime(time.time()))  # 年-月-日 时：分：秒
            msg_text = (u'[告警]问题开始时间：%s\n触发器名称：%s重启失败(超频5次)\n主机：【%s-%s】 \n严重性：High'
                        % (error_date, app_name,app_name, host_ip))  # 格式化短信内容
            send_msg(msg_text,msg_ip_port) #socket发送内容到短信触发端
            update_last_startup_time(u'Start_Error_'+dates, u'0000', u'0000', host_ip, app_name)#更新重启时间，耗时，状态码
            update_status(u'0', host_ip, app_name)  # 更新状态，离线
            logs(u'%s Restart %s Unsuccessfully' % (error_date, app_name))
            break
        else:
            time.sleep(10)
            counts += 1 #等待5秒计数加1
...
def main():
    ...
    for i in app_info():#循环列出每个应用对应的url,应用名，重启命令
        appurl = u'http://' + i[1] + u':' + unicode(i[2])#url
        # print(appurl)
        app_name = i[3]#应用名
        # print(app_name)
        restart_cmd = i[4]#重启命令
        # print(restart_cmd)
        home_path = i[4].split(u' ')[1] + i[3]#应用程序主目录
        ...
        if code == 200: #200为正常，不做操作
            ...
            update_status(u'0', host_ip, app_name)#更新状态码未0，离线
            ...
            if cluster_num > 1:#应用属于集群
                if online_status_percent >= 50:#在线率大于50%重启
                    ...
                    error_date = time.strftime(u'%Y-%m-%d %H:%M:%S', time.localtime(time.time()))  # 年-月-日 时：分：秒
                    msg_text = (u'[告警]问题开始时间：%s\n触发器名称：%s应用在线不足50%s\n主机：【%s-%s】 \n当前值：【%s】\n严重性：High'
                                % (error_date, app_name,u'%',app_name, host_ip,unicode(online_status_percent)+u'%'))  # 格式化短信内容
                    send_msg(msg_text,msg_ip_port) #socket发送内容到短信触发端
                    logs(u'[%s] 项目在线率不足50%s,当前值：%s'%(app_name,u'%',unicode(online_status_percent)+u'%'))
                    # time.sleep(60)
            else:#应用不属于集群直接重启
