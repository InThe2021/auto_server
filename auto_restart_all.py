...
def app_info():#获取app的hostname,ip,port,application,cmds信息
    ...
    with connects.cursor() as cursor:
        ...
        cursor.execute('select hostname,ip,port,application,cmds from app_info where ip="%s";' % host_ip)  # 查询数据库项目列表
        connects.commit()
...
def cluster_status(app_name):#查询app_name是否集群
    ...
    with connects.cursor() as cursor:
        cursor = connects.cursor()
        cursor.execute('select count(1) from app_info where application="%s";' % app_name)  # 查询数据库项目列表
        connects.commit()
...
def online_status(app_name):#查询app_name在线数率
    ...
    with connects.cursor() as cursor:
        cursor = connects.cursor()
        cursor.execute('select count(1) from app_info where application="%s" and status=1;' % app_name)  # 查询数据库项目列表
        connects.commit()
        online_num = cursor.fetchone()

        cursor.execute('select count(1) from app_info where application="%s" and status=0;' % app_name)  # 查询数据库项目列表
        connects.commit()
...
def update_status(status,host_ip,app_name): #更新状态
    connects = mysql_conn(dbip, username, pws, dbname)
    with connects.cursor() as cursor:
        cursor.execute("update app_info set status=%s where ip='%s' and application='%s';" % (status,host_ip,app_name))  # 更新状态
        connects.commit()
        connects.close()
...
def update_scan_err_time(dates,host_ip,app_name): #更新last_scan_err_time
    connects = mysql_conn(dbip, username, pws, dbname)
    with connects.cursor() as cursor:
        cursor.execute("update app_info set before_scan_err_time=last_scan_err_time where ip='%s' and application='%s';" % (host_ip, app_name))  # 更新before_scan_time
        connects.commit()
        cursor.execute("update app_info set last_scan_err_time='%s' where ip='%s' and application='%s';" % (dates,host_ip,app_name))  # 更新更新last_scan_time状态
        connects.commit()
        connects.close()
...
def update_last_startup_time(start_time,times,http_code,host_ip,app_name): #更新last_startup_time
    connects = mysql_conn(dbip, username, pws, dbname)
    with connects.cursor() as cursor:
        cursor.execute("update app_info set before_startup_time = last_startup_time where ip='%s' and application='%s';" % (host_ip, app_name))  # before_startup_time
        connects.commit()
        cursor.execute("update app_info set last_startup_time = '%s',startup_times = %s,http_code = %s where ip='%s' and application='%s';" % (start_time,times,http_code,host_ip,app_name))  # 更新更新start_time,times,http_code
        connects.commit()
        connects.close()
...
if __name__ == "__main__":
    ...
    pws = 'zabbix@2017'
    dbname = 'zabbix'
    msg_ip_port = ('10.0.214.222',10056)
    while True:
